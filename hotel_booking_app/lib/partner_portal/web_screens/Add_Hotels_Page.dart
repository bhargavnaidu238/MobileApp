...
class _AddHotelsPageState extends State<AddHotelsPage> with SingleTickerProviderStateMixin {
  ...
  final List<String> hotelTypes = ['Hotel', 'Resort', 'PG', 'Other'];

  String? selectedCustomization;
  final List<String> customization = ['Yes', 'No'];
  ...
  final List<String> roomTypeOptions = ['Standard Room', 'Executive Room', 'Suite Room'];
  ...
  Widget buildTextField(String field) {
    bool isNumber = ["Total_Rooms", "Pincode"].contains(field);
    bool isPhone = field == "Hotel_Contact";
    ...
    return Column(
      ...
      children: [
        DropdownButtonFormField<String>(
          ...
          items: hotelTypes.map((type) => DropdownMenuItem(value: type, child: Text(type))).toList(),
          onChanged: (val) => setState(() => selectedHotelType = val),
          decoration: InputDecoration(
        ...
        DropdownButtonFormField<String>(
          ...
          items: customization.map((type) => DropdownMenuItem(value: type, child: Text(type))).toList(),
          onChanged: (val) => setState(() => selectedCustomization = val),
          decoration: InputDecoration(
  ...
  Widget buildPreviewCard() {
    return AnimatedContainer(
      ...
      child: Column(
        ...
        children: [
          Text(
            controllers["Hotel_Name"]?.text.isEmpty ?? true ? "Hotel Name" : controllers["Hotel_Name"]!.text,
          ...
          const SizedBox(height: 8),
          Text(selectedHotelType == null ? "" : "Type: $selectedHotelType", style: const TextStyle(color: Colors.black54)),
          const SizedBox(height: 8),
          Text(_displayPriceSummary(), style: const TextStyle(color: Colors.black54)),
          Text(selectedCustomization == null ? "" : "Customization: $selectedCustomization", style: const TextStyle(color: Colors.black54)),
          const SizedBox(height: 8),
  ...
  Future<void> saveHotel() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text("Do you want to save this hotel and upload selected images?"),
        actions: [
    ...
    try {
      final Map<String, dynamic> body = {
        'hotel_id': widget.hotelData?['Hotel_ID'] ?? '',
        'partner_id': widget.partnerId,
        'hotel_name': controllers["Hotel_Name"]?.text.trim() ?? '',
        ...
        'total_rooms': controllers["Total_Rooms"]?.text.trim() ?? '',
        'available_rooms': availableRoomsController.text.trim().isEmpty
            ? controllers["Total_Rooms"]?.text.trim() ?? ''
        ...
        'hotel_contact': controllers["Hotel_Contact"]?.text.trim() ?? '',
      ...
      if (res.statusCode == 200) {
        ...
        if (decoded['status'] == 'success' || decoded['status'] == true) {
          _showSnack("Hotel saved successfully");
      ...
      _showSnack("Error saving hotel: $e");
    } finally {
                            ...
                            children: [
                              ...
                              Padding(
                                ...
                                child: TextFormField(
                                  ...
                                  onTap: pickLocation,
                                  decoration: InputDecoration(
                                    labelText: "Hotel Location (Click to select on map)",
                                    filled: true,
                                    fillColor: Colors.white,
                                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                                    focusedBorder: OutlineInputBorder(
                                    ...
                                    child: GradientButton(
                                      ...
                                      child: isSaving
                                        ...
                                        children: const [
                                          ...
                                          : const Text("Add Hotel", style: TextStyle(color: Colors.white)),
                                    ),
                                  ),
                                ],
                              ),
...
class _MapPickerPageState extends State<MapPickerPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Pick Hotel Location"),
        backgroundColor: Colors.greenAccent.shade700,
        actions: [
          TextButton(
            onPressed: () {
