...
class _HotelPaymentPageState extends State<HotelPaymentPage> {
  ...
  // To link booking with transaction
  ...
  Future<void> _startRazorpayCheckout() async {
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        // FIX: Capture the payment_record_id generated by backend to store in bookings_info
        ...
        var options = {
          ...
          'prefill': {
            'contact': widget.bookingData['mobile'] ?? '', // Corrected key case
  ...
  Future<void> _confirmPayment(String paymentType,
    ...
    // Unified key mapping to match SQL exactly
    ...
    if (isPayAtHotel) {
      ...
      booking["booking_status"] = "PENDING"; // Matches CHECK constraint
      ...
      booking["booking_status"] = "CONFIRMED"; // Matches CHECK constraint after successful pay
      booking["final_payable_amount"] = _finalPayable;
        ...
        if (!isPayAtHotel) {
          ...
          await http.post(
            ...
            body: jsonEncode({
              ...
              "user_id": booking["user_id"],     // Corrected Case
              "partner_id": booking["partner_id"], // Corrected Case
              "hotel_id": booking["hotel_id"],     // Corrected Case
              "payment_record_id": _lastPaymentRecordId, // CRITICAL for SQL Link
